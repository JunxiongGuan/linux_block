	...
	if (sense->error_code == 0x70) {
		...
		case ILLEGAL_REQUEST:
			...
			if (failed_command == NULL ||
			    req_is_passthrough(failed_command))
				break;
...
{
	/*
	 * For REQ_OP_ATA_SENSE, "rq->special" points to the original
...
static int cdrom_decode_status(ide_drive_t *drive, u8 stat)
{
	...
	if (rq->op == REQ_OP_SCSI && !rq->errors)
...
static ide_startstop_t cdrom_newpc_intr(ide_drive_t *drive)
{
	...
	int sense = (rq->op == REQ_OP_ATA_SENSE);
	...
	/* pad, if necessary */
	if (len > 0) {
		if (req_is_passthrough(rq) || write == 0)
